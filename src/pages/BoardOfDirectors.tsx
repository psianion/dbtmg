...
const BoardOfDirectors = () => {
  ...
  const fetchBoardMembers = async () => {
    try {
      ...
      if (!data) {
        console.warn("No board members found");
        setLoading(false);
        return;
      }
      ...
      const mapped = data.map((item) => ({
        ...
        directorCategory: item.category,
        bio: item.shortBiography,
        rank: item.rank,
        image: item.profileImage,
      }));

      //group by directorCategory
      const grouped = mapped.reduce((acc, person) => {
      ...
      console.error("Unexpected error fetching board members:", error);
    } finally {
      setLoading(false);
    }
  };
